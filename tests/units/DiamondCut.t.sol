...
contract Test_DiamondCut is Fixture {
  ...
  function test_RevertWhen_NoSelectorsProvidedForFacetForCut() public {
    ...
    facetCut[0] =
      FacetCut({ facetAddress: address(pureFacet), action: FacetCutAction.Add, functionSelectors: new bytes4[](0) });
  ...
  function test_RevertWhen_IncorrectFacetCutAction() public {
    ...
    facetCut[0] =
      FacetCutAux({ facetAddress: address(pureFacet), action: uint8(4), functionSelectors: new bytes4[](0) });
  ...
  function test_RevertWhen_ReplaceFromZeroAddress() public {
    FacetCut[] memory facetCut = new FacetCut[](1);
    facetCut[0] = FacetCut({ facetAddress: address(0), action: FacetCutAction.Replace, functionSelectors: selectors });
  ...
  function test_RevertWhen_ReplaceWithoutChange() public {
    ...
    vm.expectRevert(
      abi.encodeWithSelector(Errors.CannotReplaceFunctionWithTheSameFunctionFromTheSameFacet.selector, selectors[0])
    );
  ...
  function test_RevertWhen_RemoveFacetAddressMustBeZeroAddress() public {
    FacetCut[] memory facetCut = new FacetCut[](1);
    facetCut[0] =
      FacetCut({ facetAddress: address(pureFacet), action: FacetCutAction.Remove, functionSelectors: selectors });
  ...
  function test_RevertWhen_RemoveImmutableFunction() public {
    ...
    FacetCut[] memory facetCut = new FacetCut[](1);
    facetCut[0] =
      FacetCut({ facetAddress: address(0), action: FacetCutAction.Remove, functionSelectors: functionSelectors });
