...
contract SavingsNameableUpgradeTest is Test, Helper {
    uint256 constant CHAIN = Constants.CHAIN_ETHEREUM;
    ...
    function setUp() public {
        ...
        savings = _chainToContract(CHAIN, ContractType.sUSDp);


        proxyAdmin = ProxyAdmin(_chainToContract(CHAIN, ContractType.ProxyAdmin));
        governor = _chainToContract(CHAIN, ContractType.DaoMultisig);
        ...
        assertEq(IERC20Metadata(savings).name(), "Staked EURp");
        assertEq(IERC20Metadata(savings).symbol(), "sEURp");
        ...
        previewDeposit = SavingsNameable(savings).previewDeposit(Constants.BASE_18);
        previewMint = SavingsNameable(savings).previewMint(Constants.BASE_18);
        previewWithdraw = SavingsNameable(savings).previewWithdraw(Constants.BASE_18);
        previewRedeem = SavingsNameable(savings).previewRedeem(Constants.BASE_18);
    ...
    function _upgradeContract(string memory name, string memory symbol) internal {
        ...
        proxyAdmin.upgradeAndCall(ITransparentUpgradeableProxy(payable(savings)), savingsImpl, "");
        vm.prank(governor, governor);
    ...
    function test_UpdatedValues() public {
        ...
        assertEq(SavingsNameable(savings).previewRedeem(Constants.BASE_18), previewRedeem);
        assertEq(SavingsNameable(savings).previewWithdraw(Constants.BASE_18), previewWithdraw);
        assertEq(SavingsNameable(savings).previewMint(Constants.BASE_18), previewMint);
        assertEq(SavingsNameable(savings).previewDeposit(Constants.BASE_18), previewDeposit);
