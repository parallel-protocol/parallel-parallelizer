...
/// @title GenericRebalancerUtils
/// @notice Utility functions for GenericRebalancer testing

abstract contract GenericRebalancerUtils is Test {
  ...
  /// @notice Deploy and configure a GenericRebalancer with a given router
  ...
  function _deployAndConfigureHarvester(
    ...
    internal
    returns (GenericRebalancer harvester)
  {
    ...
    // Deploy harvester
    vm.prank(governor);
    harvester = new GenericRebalancer(
  ...
  function _setupUserBudget(GenericRebalancer harvester, address user, address tokenP, uint256 budgetAmount) internal {
  ...
  function _createSwapData(address eurB, address eurA) internal pure returns (bytes memory extraData) {
    return _createSwapData(eurB, eurA, 1_500_000);
  }

  /// @notice Create swap data for harvest with a configurable output amount
  /// @param eurB Address of eurB token
  /// @param eurA Address of eurA token
