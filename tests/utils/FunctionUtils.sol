...
contract FunctionUtils is StdUtils {
  ...
  function _generateCurves(
    ...
    internal
    pure
    returns (uint64[] memory postThres, int64[] memory postIntercep)
  {
          ...
          mint
            && (
              int256(BASE_9) <= int256(intercepts[i])
                || int256(BASE_18) / (int256(BASE_9) - int256(intercepts[i])) - int256(BASE_9) >= int256(BASE_12) - 1
            )
        ) || intercepts[i] == int256(BASE_9) || (increasing && thresholds[i] == BASE_9 - 1)
